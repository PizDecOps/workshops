...
1. **I/O Redirection:** Потоки `stdin`, `stdout`, `stderr` и магия дескрипторов.
2. **Переменные:** Local, Global (export) и встроенные переменные окружения.
3. **Кавычки и экранирование:** Разница между `'`, `"` и `` ` ``.
4. **Арифметика:** Математика в `(( ))` и внешние утилиты `bc`, `expr`.
5. **Условия (Tests):** Сравнение строк и чисел в `[[]]`.
6. **Текстовые процессоры:** Фильтрация через `grep`, `sed` и `awk`.
7. **Конвейеры (Pipelines):** Как строить цепочки обработки данных.
8. **Циклы:** Автоматизация через `for`, `while` и `until`.
9. **Функции:** Группировка кода и передача аргументов `$1`, `$2`.
10. **Обработка сигналов:** `trap` для очистки мусора при выходе.
11. **Скрипты-обертки:** Использование `set -e`, `set -x` и `getopts`.
12. **Exit Codes:** Как понимать, почему всё упало (или нет).
